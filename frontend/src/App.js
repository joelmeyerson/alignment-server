...
export default function AlignmentApp() {
  ...
  async function fetchAlignments() {
    await fetch("http://localhost:8000/api/alignment")
  ...
  async function handleSubmit(e) {
    ...
    if (dna === "") {
      ...
      // POST to server
      const response = await fetch("http://localhost:8000/addAlignment/", {
        method: "POST",
        headers: {
            ...
            if (alignment.position > 0) {
              ...
              return (
                <div key={alignment.id} className="py-2 my-1 overflow-auto">
                  ...
                  <p>
                    <span className="font-semibold">Protein sequence:</span> <span className="text-red-400">{alignment.protein_sequence}</span>
                  </p>
                </div>
              );
            }
